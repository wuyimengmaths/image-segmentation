function [c1,c2,u,S,B] = seg_gcs(u,Img,rho1,lameda,alpha,beta,rho,rho2,eps,maxiter,type,step)
[M,N] = size(Img);
B = zeros(M,N);
S = Img-B;
theta = zeros(M,N);
...
for ii=1:maxiter
    ...
    if (norm2D(u-uprev,2)<=eps) && (ii>=step)
       break;
   end
...
S=real( idct2( dct2((theta+rho*(Img-B)-2*lameda*(u.*c2-u.*c1-c2))./rho2+(DIV(d2.*rho2,theta2))./rho2)./SfiltDCT));%solve S
